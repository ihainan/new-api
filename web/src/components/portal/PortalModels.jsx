...
const CAP_HELP = {
  stream: '生成过程中逐段返回，不必等整段写完。在请求中设置 stream: true 即可。',
  tools:
    '由你提供可调用的函数定义，模型决定何时调用以及参数取值，实际执行仍由你的代码完成。',
  json: '通过 response_format 约束输出，使返回内容只包含 JSON，不夹杂说明文字。',
  vision: '消息中可以附带图片，模型会结合图片内容作答。',
  reasoning: '回答前会先进行推理，返回结果中可以取到思考过程。',
  cache: '相同的前缀（例如较长的系统提示或同一份文档）会命中缓存，第二次起响应更快、开销更小。',
};
