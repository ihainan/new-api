...
export const MODELS = [
  ...
  },
  {
    ...
    detail:
      '当前指向 GLM-5.2，适用于日常问答、文本改写、内容总结和代码辅助。上下文 1M token，是平台上最长的，需要一次性读入长文档或整个代码库时优先选它。**不支持图片输入：带图片的请求不会报错，但模型看不到图片内容。**`glm` 这个模型 ID 长期保持不变，实际指向的模型版本会随上游升级而变化。',
  ...
  },
  {
    ...
    detail:
      '总参数 35B，单次推理约激活 3B，在质量和速度之间取得平衡。上下文 256K token，足以一次读入长文档或整份代码。该模型会输出思考过程，字段名为 `reasoning`，**与 `glm` 使用的 `reasoning_content` 不同**，解析时需要区分。`qwen` 这个模型 ID 长期保持不变，实际指向的模型版本会随上游升级而变化。',
  ...
  },
  {
    ...
    detail:
      '总参数 26B，单次推理约激活 4B，支持图片输入。它会先输出思考过程（字段名 `reasoning_content`）再给出正文，因此 **`max_tokens` 需要留足余量，否则思考过程会占满额度，正文返回为空**。权重公开，允许本地微调与再分发，但须随附 Gemma 使用条款并遵守其使用政策，不同于 MIT、Apache 这类宽松许可。上下文 4K token（中文约 6,800 字），**超出后仅保留约 2K token 且不会报错**，长文本任务请改用 `glm` 或 `qwen`。',
  ...
  },
  {
    ...
    detail:
      '当前指向 Qwen-Image-2.0，根据文字描述生成图片。未指定尺寸时输出 1024×1024，采样 30 步。中文提示词可以直接使用，画面内的中文也能正确渲染，制作海报、配图和示意图无需先译成英文。提示词最长 1K token，足够写清楚细节。',
  ...
  },
  {
    ...
    detail:
      '当前指向 Qwen3-ASR-1.7B，将音频转写为文本，支持中文以及中英混合口语，常用于会议录音、访谈整理和视频字幕。单次请求最多处理约 **157 秒**音频，更长的文件会在静音处自动切段，分别转写后拼接返回。接口与 OpenAI 的 `audio/transcriptions` 兼容。',
    params: '1.7B 参数',
    context: null,
    io: '音频 → 文本',
  },
  ...
  },
  {
    ...
    detail:
      '当前指向 MiniMax-H3，根据文字描述生成视频，也支持由单张图片生成。未指定参数时输出 4 秒、9:16、短边 768 的视频。生成耗时通常在几分钟，接口为异步任务形式：提交后先取回任务 ID，再轮询结果，**不要按同步请求设置超时**。任务进度可在「使用记录 → 任务」中查看。',
    params: '未公布',
    context: null,
    io: '文本 / 图像 → 视频',
  },
