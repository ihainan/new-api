...
export const MODELS = [
  ...
  },
  {
    ...
    detail:
      '`minimax-h3` 当前指向 MiniMax-H3：33B 的全模态生成模型，按文字描述生成带声音的视频，也支持由单张图片生成，模型本身覆盖 4～15 秒。**本平台默认**输出 4 秒、9:16、短边 768。生成耗时通常在几分钟，接口为异步任务：向 `/v1/videos` 提交后取回任务 ID，再轮询结果，**不要按同步请求设置超时**。任务进度可在「任务队列」页查看，那页在任务跑完前会自动刷新。',
